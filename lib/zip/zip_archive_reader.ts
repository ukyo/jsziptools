import {BufferLike, concatBytes, detectEncoding, bytesToString, toBytes, readFileAsText, readFileAsDataURL, ENV_IS_WORKER} from '../common';
...
export abstract class ZipArchiveReader {
    ...
    readFileAsArrayBufferSync(fileName: string) {
        if (!ENV_IS_WORKER) throw new Error('ZipArchiveReader#readFileAsArrayBufferSync is available on only a worker process.');
    ...
    readFileAsDataURLSync(fileName: string) {
        if (!ENV_IS_WORKER) throw new Error('ZipArchiveReader#readFileAsDataURLSync is available on only a worker process.');
