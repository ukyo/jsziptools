import {BufferLike, toBytes, LOCAL_FILE_SIGNATURE, DATA_DESCRIPTOR_SIGNATURE, CENTRAL_DIR_SIGNATURE, END_SIGNATURE, defun} from '../common';
...
class _ZipArchiveWriter {
    ...
    constructor(shareMemory?: boolean, chunkSize?: number)
    constructor(readonly shareMemory?: boolean, readonly chunkSize?: number) {
    }
...
function createLocalFileHeader(fileName: Uint8Array, date: Date, isDeflated: boolean) {
    ...
    let offset = 0;
    view.setUint32(offset, LOCAL_FILE_SIGNATURE, true); offset += 4; // local file header signature
...
function createCentralDirHeader(fileName: Uint8Array, date: Date, isDeflated: boolean, fileOffset: number, uncompressedSize: number, compressedSize: number, crc: number) {
    let view = new DataView(new ArrayBuffer(46 + fileName.length));
    let bytes = new Uint8Array(view.buffer);
    let offset = 0;
    view.setUint32(offset, CENTRAL_DIR_SIGNATURE, true); offset += 4; // central file header signature
...
function createEndCentDirHeader(numberOfCentralDirs: number, centralDirHeaderSize: number, centralDirStartOffset: number) {
